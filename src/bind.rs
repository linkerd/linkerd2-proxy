...
/// An HTTP `Service` that is created for each `Endpoint` and `Dialect`.
pub type Stack<B> = proxy::http::orig_proto::Upgrade<
    proxy::http::NormalizeUri<
        WatchTls<B>
    >
>;
...
impl<B> MakeClient<Endpoint> for BindDialect<ctx::Proxy, B>
where
    B: tower_h2::Body + Send + 'static,
    <B::Data as ::bytes::IntoBuf>::Buf: Send,
{
