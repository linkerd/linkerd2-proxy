...
fn metrics_endpoint_outbound_request_count() {
    ...
    // prior to seeing any requests, request count should be empty.
    assert!(!metrics.get("/metrics")
        .contains("request_total{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"}"));

    info!("client.get(/)");
    assert_eq!(client.get("/"), "hello");

    // after seeing a request, the request count should be 1.
    assert_contains!(metrics.get("/metrics"), "request_total{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
    ...
    fn expected_metric(status: &http::StatusCode, direction: &str, tls: &str, no_identity: Option<&str>) -> String {
        format!(
            ...
            if let Some(no_identity) = no_identity {
                format!("no_identity=\"{}\",", no_identity)
            } else {
                String::new()
            },
...
fn metrics_endpoint_outbound_response_latency() {
    ...
    // extra latency.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_bucket{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\",le=\"1000\"} 1");
    // the histogram's count should be 1.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_count{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
    // TODO: we're not going to make any assertions about the
    // response_latency_ms_sum stat, since its granularity depends on the actual
    // observed latencies, which may vary a bit. we could make more reliable
    // assertions about that stat if we were using a mock timer, though, as the
    ...
    // request with 40ms extra latency should fall into the 50ms bucket.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_bucket{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\",le=\"50\"} 1");
    // 1000ms bucket should be incremented as well, since it counts *all*
    // bservations less than or equal to 1000ms, even if they also increment
    // other buckets.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_bucket{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\",le=\"1000\"} 2");
    // the histogram's total count should be 2.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_count{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 2");
    ...
    // request with 40ms extra latency should fall into the 50ms bucket.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_bucket{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\",le=\"50\"} 2");
    // 1000ms bucket should be incremented as well.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_bucket{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\",le=\"1000\"} 3");
    // the histogram's total count should be 3.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_count{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 3");
    ...
    // 1000ms bucket should be incremented.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_bucket{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\",le=\"1000\"} 4");
    // the histogram's total count should be 4.
    assert_contains!(metrics.get("/metrics"),
        "response_latency_ms_count{authority=\"tele.test.svc.cluster.local\",direction=\"outbound\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 4");
}
...
mod outbound_dst_labels {
    ...
    fn labeled_addr_and_addrset() {
        ...
        assert_eq!(client.get("/"), "hello");
        assert_contains!(metrics.get("/metrics"),
            "response_latency_ms_count{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"foo\",dst_set_label=\"bar\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "request_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"foo\",dst_set_label=\"bar\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "response_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"foo\",dst_set_label=\"bar\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
    }
    ...
    fn controller_updates_addr_labels() {
        ...
        assert_contains!(metrics.get("/metrics"),
            "response_latency_ms_count{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"foo\",dst_set_label=\"unchanged\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "request_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"foo\",dst_set_label=\"unchanged\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "response_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"foo\",dst_set_label=\"unchanged\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        ...
        assert_contains!(metrics.get("/metrics"),
            "response_latency_ms_count{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"bar\",dst_set_label=\"unchanged\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "request_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"bar\",dst_set_label=\"unchanged\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "response_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_addr_label=\"bar\",dst_set_label=\"unchanged\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
    ...
    fn controller_updates_set_labels() {
        ...
        // the first request should be labeled with `dst_addr_label="foo"`
        assert_contains!(metrics.get("/metrics"),
            "response_latency_ms_count{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"foo\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "request_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"foo\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "response_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"foo\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        ...
        // the second request should increment stats labeled with `dst_addr_label="bar"`
        assert_contains!(metrics.get("/metrics"),
            "response_latency_ms_count{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"bar\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "request_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"bar\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
        assert_contains!(metrics.get("/metrics"),
            "response_total{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"bar\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        // stats recorded from the first request should still be present.
        assert_contains!(metrics.get("/metrics"),
            "response_latency_ms_count{authority=\"labeled.test.svc.cluster.local\",direction=\"outbound\",dst_set_label=\"foo\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\",classification=\"success\",status_code=\"200\"} 1");
        assert_contains!(metrics.get("/metrics"),
...
mod transport {
    ...
    fn inbound_http_connect() {
        ...
        assert_eq!(client.get("/"), "hello");
        assert_contains!(metrics.get("/metrics"),
            "tcp_open_total{direction=\"inbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
    ...
    fn outbound_http_connect() {
        ...
        assert_eq!(client.get("/"), "hello");
        assert_contains!(metrics.get("/metrics"),
            "tcp_open_total{direction=\"outbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_provided_by_service_discovery\"} 1");
    ...
    fn inbound_tcp_connect() {
        ...
        assert_contains!(metrics.get("/metrics"),
            "tcp_open_total{direction=\"inbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\"} 1");
    }
    ...
    fn inbound_tcp_duration() {
        ...
        assert_contains!(out,
            "tcp_connection_duration_ms_count{direction=\"inbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\",classification=\"success\"} 1");
        ...
        assert_contains!(out,
            "tcp_connection_duration_ms_count{direction=\"inbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\",classification=\"success\"} 2");
    }
    ...
    fn inbound_tcp_write_bytes_total() {
        ...
        let dst_expected = format!(
            "tcp_write_bytes_total{{direction=\"inbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\"}} {}",
    ...
    fn inbound_tcp_read_bytes_total() {
        ...
        let dst_expected = format!(
            "tcp_read_bytes_total{{direction=\"inbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\"}} {}",
            TcpFixture::BYE_MSG.len()
        );
    ...
    fn outbound_tcp_connect() {
        ...
        assert_eq!(tcp_client.read(), TcpFixture::BYE_MSG.as_bytes());
        assert_contains!(metrics.get("/metrics"),
            "tcp_open_total{direction=\"outbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\"} 1");
    }
    ...
    fn outbound_tcp_duration() {
        ...
        assert_contains!(out,
            "tcp_connection_duration_ms_count{direction=\"outbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\",classification=\"success\"} 1");
        ...
        assert_contains!(out,
            "tcp_connection_duration_ms_count{direction=\"outbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\",classification=\"success\"} 2");
    }
    ...
    fn outbound_tcp_write_bytes_total() {
        ...
        let dst_expected = format!(
            "tcp_write_bytes_total{{direction=\"outbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\"}} {}",
            TcpFixture::HELLO_MSG.len()
        );
    ...
    fn outbound_tcp_read_bytes_total() {
        ...
        let dst_expected = format!(
            "tcp_read_bytes_total{{direction=\"outbound\",peer=\"dst\",tls=\"no_identity\",no_identity=\"not_http\"}} {}",
            TcpFixture::BYE_MSG.len()
        );
