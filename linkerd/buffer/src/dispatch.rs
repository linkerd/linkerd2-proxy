        ...
        loop {
            ...
            match ready {
                ...
                Poll::Ready(Err(error)) => {
                    let error = ServiceError(Arc::new(error.into()));
                    trace!(%error, "Inner service failed");

                    // First, notify services of the readiness change to prevent new requests from
                    // being buffered.
                    let broadcast = this.ready.broadcast(Poll::Ready(Err(error.clone())));
            ...
            match this.rx.poll_recv(cx) {
                Poll::Pending => {
                    ...
                    if let Some(idle) = this.current_idle.as_mut() {
                        if idle.poll_unpin(cx).is_ready() {
                            let error = IdleError(*this.max_idle.as_ref().unwrap());

                            // Drop the inner Service to free its resources. It won't be used again.
                            trace!(%error, "Dropping buffered service");
                            let _ = this.inner.take();
                            ...
                            return_ready_if!(this
                                .ready
                                .broadcast(Poll::Ready(Err(shared)))
                                .is_err());
                        }
                    }
...
mod test {
    ...
    async fn idle_when_unused() {
        ...
        let mut dispatch = task::spawn(super::Dispatch::new(inner, rx, ready_tx, Some(max_idle)));
        handle.allow(1);

        // Service ready without requests. Idle counter starts ticking.
        assert!(dispatch.poll().is_pending());
