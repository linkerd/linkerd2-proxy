...
impl<S> Stack<S> {
    ...
    pub fn push_idle_cache<K, T, Svc>(
        self,
        idle: Duration,
    ) -> Stack<idle_cache::NewIdleCached<K, S, Svc>>
    where
        K: Clone + Eq + std::fmt::Debug + std::hash::Hash + Send + Sync + 'static,
        S: NewService<T, Service = Svc> + 'static,
