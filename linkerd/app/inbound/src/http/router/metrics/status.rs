...
use linkerd_http_prom::{
    status,
    stream_label::{
        status::{LabelGrpcStatus, MkLabelGrpcStatus},
...
pub enum LabelStatus {
    ...
    #[allow(dead_code, reason = "XXX(kate): work in progress")]
    Http {
        // mk_label_http: (),
        // route: RouteLabels,
    },
    ...
    T: svc::Param<PermitVariant> + svc::Param<RouteLabels>,
{
    fn extract_param(&self, target: &T) -> Params {
        ...
        let mk_stream_label = match variant {
            ...
            PermitVariant::Http => MkLabelStatus::Http {},
        };
    ...
    fn status_labels(&self) -> Self::StatusLabels {
        match self {
            Self::Grpc { label_grpc, route } => {
                ...
                let status = label_grpc.status_labels().expect("XXX(kate)") as u16; // XXX
