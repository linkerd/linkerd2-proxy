...
async fn grpc_response_class() {
    ...
    let connect = {
        ...
        support::connect().endpoint_fn_boxed(
            Target::addr(),
            grpc_error_server(server, tonic::Code::Unknown),
        )
    };
    ...
    // XXX NOWAY FIXME
    let metrics = {
        let m = metrics.as_display().to_string();
        m.lines().map(|v| v.to_string()).collect::<Vec<_>>()
    };
    assert_eq!(*metrics.last().unwrap(), "response_total{direction=\"inbound\",authority=\"foo.svc.cluster.local:5550\",target_addr=\"127.0.0.1:80\",target_ip=\"127.0.0.1\",target_port=\"80\",tls=\"true\",client_id=\"foosa.barns.serviceaccount.identity.linkerd.cluster.local\",srv_group=\"policy.linkerd.io\",srv_kind=\"server\",srv_name=\"testsrv\",route_group=\"\",route_kind=\"default\",route_name=\"default\",authz_group=\"policy.linkerd.io\",authz_kind=\"serverauthorization\",authz_name=\"testsaz\",status_code=\"200\",classification=\"failure\",grpc_status=\"2\",error=\"\"} 1");
...
#[tracing::instrument]
fn grpc_error_server(
    http: hyper::server::conn::Http,
    status: tonic::Code,
) -> impl Fn(Remote<ServerAddr>) -> io::Result<io::BoxedIo> {
    move |endpoint| {
        let span = tracing::info_span!("grpc_error_server", ?endpoint);
        let _e = span.enter();
        tracing::info!("mock connecting");
        let (client_io, server_io) = support::io::duplex(4096);
        tokio::spawn(
