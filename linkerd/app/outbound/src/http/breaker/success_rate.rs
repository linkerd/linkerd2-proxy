...
//! ring of fixed-duration buckets that together span the configured `decay`
//! window. Each bucket counts successes and totals for the responses that land in
//! its time slice, so the ratio is the exact quotient of the live buckets and
//! does not depend on how fast requests arrive.
//!
...
//! ## Window mechanics
//!
//! The window holds `BUCKETS` slices, each `bucket_width` wide, covering `decay`.
...
/// Number of buckets spanning the decay window. A fixed internal constant: ten
/// slices give the ratio enough time resolution to expire old samples smoothly
/// without making the per-response roll or the per-check sum expensive.
const BUCKETS: usize = 10;

/// Floor for a single bucket's width. With a very short decay the per-bucket
/// width could round to zero, which would make ring advancement divide by zero.
/// One millisecond matches the moving average's own decay floor.
...
/// bucket per slice. The window is `bucket_width * BUCKETS`, so flooring `decay`
...
pub(crate) struct SuccessRateWindow {
    ...
    /// Width of one bucket. `decay / BUCKETS`, floored at [`MIN_BUCKET_WIDTH`].
...
impl SuccessRateWindow {
    /// Build a window spanning `decay`, anchored at `now`.
    ///
    /// The realized window is `bucket_width * BUCKETS`, so the decay is floored at
    /// [`MIN_DECAY`] before the division rather than flooring each bucket. The
    /// realized window then equals the configured `decay` exactly for any value at
    /// or above the floor, and the floor keeps the bucket width non-zero for a
    /// shorter decay the config layer would otherwise reject.
    pub(crate) fn new(decay: Duration, now: Instant) -> Self {
        let bucket_width = decay.max(MIN_DECAY) / BUCKETS as u32;
        Self {
...
mod tests {
    ...
    async fn bucket_width_floors_at_minimum() {
        let now = Instant::now();
        // A sub-millisecond decay would divide to a zero-width bucket, so the floor
        // keeps the ring usable.
        let w = SuccessRateWindow::new(Duration::from_micros(100), now);
        assert_eq!(w.bucket_width, MIN_BUCKET_WIDTH);
    }

    // The realized window is `bucket_width * BUCKETS`, and flooring the decay
    // rather than the bucket keeps it equal to the configured decay for any value
    // at or above the floor. A 5s decay realizes a 5s window exactly, and a decay
    // at the floor realizes the floor rather than rounding up.
    #[tokio::test(flavor = "current_thread", start_paused = true)]
    async fn realized_window_equals_configured_decay() {
        let now = Instant::now();

        let decay = Duration::from_secs(5);
        let w = SuccessRateWindow::new(decay, now);
        assert_eq!(
            w.bucket_width * BUCKETS as u32,
            decay,
            "a decay above the floor realizes a window of exactly that decay",
        );

        // At the floor the realized window is the floor, not a wider span.
        let w = SuccessRateWindow::new(MIN_DECAY, now);
        assert_eq!(w.bucket_width * BUCKETS as u32, MIN_DECAY);
    ...
    // fraction of the decay trips exactly when the sample floor is met, and the
    ...
    async fn failure_burst_trips_independent_of_rate() {
        ...
        // Slow burst: the same five failures, one per second. The decay is 10s,
    ...
    #[tokio::test(flavor = "current_thread", start_paused = true)]
    async fn samples_age_out_after_decay() {
        let (mut w, now) = window_10s();
        ...
        // A response arriving a full decay later rolls the ring on record. Every
        ...
        assert_eq!(
            w.ratio(),
            Some(1.0),
            "failures older than the decay window no longer count",
        );
        assert!(
            !w.should_trip(1, 0.5),
    ...
    async fn ratio_recovers_as_failures_expire() {
        ...
        // Past the decay from the failures, only the successes remain: 8/8.
        time::advance(Duration::from_secs(11)).await;
    ...
    // out of the window is the one that gets cleared. Here decay=10s gives 1s
