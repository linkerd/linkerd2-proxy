    ...
    fn from((rts, parent): (crate::tls::Routes, T)) -> Self {
        ...
        let mk_dispatch = move |bke: &policy::Backend| match bke.dispatcher {
            policy::BackendDispatcher::BalanceP2c(
                ...
                // TLS traffic balances on round-trip time, since the penalty
                // estimator needs HTTP classification that does not apply here.
                // Read the RTT configuration from whichever estimator the policy
                // chose and drop any penalty fields, warning the operator when
                // the dropped configuration was non-trivial.
                if let Some(p) = load.dropped_penalty() {
                    tracing::warn!(
                        penalty = ?p.penalty,
                        penalty_decay = ?p.penalty_decay,
                        max_retry_after = ?p.max_retry_after,
                        "TLS balancer ignores the response-penalty estimator; only round-trip time applies",
                    );
                }
