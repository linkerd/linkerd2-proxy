use crate::core::{
    ...
    dns, identity,
    proxy::http::h2,
    transport::{listen, tls, OrigDstAddr, SysOrigDstAddr},
    Addr,
};
use crate::outbound::Config as OutboundConfig;
...
pub struct Config<O: OrigDstAddr = SysOrigDstAddr> {
    pub outbound: OutboundConfig<O>,
    pub inbound: ProxyConfig<O>,

    pub dns: DnsConfig,
    pub identity: tls::Conditional<IdentityConfig>,
    pub destination: DestinationConfig,
    pub admin: AdminConfig,
    pub tap: Option<TapConfig>,
    pub trace_collector: Option<TraceCollectorConfig>,
}
...
impl<O: OrigDstAddr> Config<O> {
    pub fn with_orig_dst_addrs_from<P: OrigDstAddr>(self, orig_dst_addrs: P) -> Config<P> {
        Config {
            outbound: self
                .outbound
                .with_orig_dst_addrs_from(orig_dst_addrs.clone()),
            inbound: self.inbound.with_orig_dst_addrs_from(orig_dst_addrs),
            dns: self.dns,
...
impl Config {
    ...
    pub fn parse<S: Strings>(strings: &S) -> Result<Self, EnvError> {
        ...
        let admin = AdminConfig {
            ...
            server: ServerConfig {
                bind: listen::Bind::new(
                    ...
                    inbound.server.bind.keepalive(),
                ),
                buffer: inbound.server.buffer,
                h2_settings,
            },
        };
        ...
        let trace_collector = trace_collector_addr?.map(|addr| {
            let (connect, buffer) = if addr.addr.is_loopback() {
                (inbound.connect.clone(), inbound.server.buffer)
            } else {
        ...
        let tap = tap?.map(|(sa, ids)| TapConfig {
            server: ServerConfig {
                bind: listen::Bind::new(sa, inbound.server.bind.keepalive()),
                buffer: inbound.server.buffer,
                h2_settings,
            },
            permitted_peer_identities: Arc::new(ids),
        });

        let identity = identity_config?.map(|(addr, identity)| {
...
pub fn parse_identity_config<S: Strings>(
    strings: &S,
) -> Result<tls::Conditional<(ControlAddr, identity::Config)>, EnvError> {
    let addr = parse_control_addr(strings, ENV_IDENTITY_SVC_BASE);
    ...
    match (
        ...
        (disabled, None, None, None, None, None, None, None) => {
            if !disabled {
                ...
                Ok(tls::Conditional::None(tls::ReasonForNoIdentity::Disabled))
            }
        }
        (
            false,
