...
type Sender = mpsc::UnboundedSender<(
    Request<BoxBody>,
    oneshot::Sender<Result<Response<http_body_util::Full<Bytes>>, ClientError>>,
)>;
...
impl Client {
    ...
    ) -> impl Future<Output = Result<Response<http_body_util::Full<Bytes>>, ClientError>> + Send + 'static
    {
        let req = builder.body(BoxBody::empty()).unwrap();
        self.send_req(req)
    }

    pub async fn request_body<B>(&self, req: Request<B>) -> Response<http_body_util::Full<Bytes>>
    where
        B: Body + Send + 'static,
        B::Data: Send + 'static,
        B::Error: Into<Error>,
...
fn run(
    ...
    let (tx, rx) = mpsc::unbounded_channel::<(
        Request<BoxBody>,
        oneshot::Sender<Result<Response<BoxBody>, ClientError>>,
    )>();
