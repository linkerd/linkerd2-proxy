...
async fn does_not_retry_if_streaming_body_exceeds_max_length() {
    ...
    profile_test! {
        ...
        with_client: |client: client::Client| async move {
            ...
            drop(tx);
            let res = res.await.unwrap();
            // The body getting capped will result in a 500 error generated by
            // hyper when the body length exceeds the limit, rather than the
            // error returned by the server in the initial request...it would be
            // nice to fix this...
