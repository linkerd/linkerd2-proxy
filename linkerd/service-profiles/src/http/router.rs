...
use linkerd2_stack::{NewService, Shared};
use std::hash::Hash;
...
type ConcreteRouter<Target, Svc, Body> =
    rt::Router<http::Request<Body>, ConcreteDstRecognize<Target>, rt::FixedNewService<Target, Svc>>;

// A router which routes based on the "route" of the target.
type RouteRouter<Target, RouteTarget, Svc, Body> =
    rt::Router<http::Request<Body>, RouteRecognize<Target>, rt::FixedNewService<RouteTarget, Svc>>;
...
where
    ...
    RouteLayer: tower::layer::Layer<
        ...
        Service = RouteMake,
    >,
    RouteMake: NewService<Target::Output>,
...
where
    ...
    RouteLayer::Service: NewService<<Target as WithRoute>::Output, Service = RouteSvc> + Clone,
    ...
    RouteSvc::Error: Into<Error>,
{
    ...
    fn call(&mut self, target: Target) -> Self::Future {
        let concrete_router = {
            ...
            make.insert(target.clone(), self.inner.new_service(target.clone()));
        ...
        let router = {
            ...
            let stack = concrete_stack.new_service(default_route.clone());
...
where
    ...
    RouteLayer: tower::layer::Layer<
            ...
            Service = RouteMake,
        > + Clone,
    RouteMake: NewService<Target::Output> + Clone,
    RouteMake::Service: tower::Service<http::Request<RouteBody>> + Clone,
    Inner: NewService<Target> + Clone,
    Inner::Service: tower::Service<http::Request<InnerBody>> + Clone,
{
    fn update_routes(&mut self, routes: Routes) {
        ...
        for WeightedAddr { addr, .. } in &routes.dst_overrides {
            ...
            let service = old_make
                .remove(&target)
                .unwrap_or_else(|| self.inner.new_service(target.clone()));
        ...
        let mut make = IndexMap::with_capacity(capacity);
        make.insert(default_route.clone(), stack.new_service(default_route));

        for (_, route) in &routes.routes {
            let route = self.target.clone().with_route(route.clone());
            let service = stack.new_service(route.clone());
...
where
    ...
    RouteLayer: tower::layer::Layer<
            ...
            Service = RouteMake,
        > + Clone,
    RouteMake: NewService<Target::Output, Service = RouteSvc> + Clone,
    Inner: NewService<Target> + Clone,
    Inner::Service: tower::Service<http::Request<InnerBody>> + Clone,
    RouteSvc: tower::Service<http::Request<RouteBody>> + Clone,
    RouteSvc::Error: Into<Error>,
{
    ...
    type Future = rt::ResponseFuture<
        ...
        rt::FixedNewService<Target::Output, RouteSvc>,
    >;
