...
b) Run './memory-profiler-cli server $OUT_DIR/CHANGEME.heap.dat' and open http://localhost:8080/ to browse the memory graphs
c) Run 'heaptrack -a $OUT_DIR/.heaptrack.dat' to open the heaptrack files for a detailed view.
(Replace CHANGEME with http1inbound, http1outbound, grpcinbound, grpcoutbound, tcpinbound, tcpoutbound.)"
